...
struct Story: AsyncParsableCommand {
    ...
    @Option(name: .shortAndLong, help: "Choose your story length: <short>, <medium> or <long>")
    var length: Length?
    
    @Option(name: .shortAndLong, help: "Choose if your child is: <toddler> (0-3 yrs old), <preschooler> (4-6 yrs old) or <kid> (7-10+ yrs old)")
    var age: Age?
    ...
    func run() async throws {
        ...
        let response = try await model.generateContent(story)
            if let text = response.text {
                printColor(color: Colors.red, text: "Once upon a time there was:")
                print(text)
                print()
            }
        }
